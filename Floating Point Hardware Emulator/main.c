...
int main(int argc, char* argv[])
{
	...
	printf("\n\n the floating point value for %g is broken out as:\n", float_32.floating_value_in_32_bits);

	printf("\tmantissa:\t\t0x%x\tor:\t\t", float_32.f_bits.mantissa);
	// print mantissa bits arr
	for (i = 11; i < 40; ++i) {
		printf("%c", bit_string[i]);
	}
	printf("\n");

	printf("\texponent:\t\t0x%x\t\tor:\t\t", float_32.f_bits.exponent);
	//print exponent bits array
	for (i = 1; i < 11; ++i) {
		printf("%c", bit_string[i]);
	}
	printf("\n");
	...
	printf("\tsign:\t\t\t0x%x\t\tor:\t\t %d\n", float_32.f_bits.sign, float_32.bit.b0);
	printf("\tin base 10:\t\t%g\t\tor:\t\t ", float_32.floating_value_in_32_bits);
	for (i = 0; i < 40; ++i) {
		printf("%c", bit_string[i]);
	}
	printf("\n");
